...
Codex is optional. The same controller is exposed through MCP, Skill, `release_gate_cli.py`, and an unattended OS scheduler. Run `py -3 src/release_gate_cli.py setup` as the final scheduler identity, then use `preflight`, `run-once`, `status`, `doctor`, and `scheduler status`. Setup creates the shared configuration without manual JSON editing, initializes or reuses the two per-user Windows Credential Manager keys, binds the process-token identity, rejects unsafe credential evidence, installs one scheduler, and returns the remaining production-preflight gaps. It never enables deployment or report automation.
...
The preferred Windows path is one setup command under the exact account that will own
the scheduled task:

```powershell
$env:PRODUCT_RELEASE_GATE_CONFIG = "C:\path\to\product-release-gate.json"
py -3 src/release_gate_cli.py --config $env:PRODUCT_RELEASE_GATE_CONFIG setup
```

Setup calls the same safe credential initializer used by
`scripts/provision_windows_credentials.py init`. It creates only missing per-user
Credential Manager values, records only their non-secret target names in JSON, never
prints values, never rotates existing credentials, and fails before scheduler
installation if identity or credential evidence is unsafe. Use the standalone `status`
command for audit and `rebind --confirm-runtime-identity-rebind` only for an approved
service-account migration. A configuration generated by the filesystem bootstrap can
also be initialized manually when setup is intentionally not used. CI
