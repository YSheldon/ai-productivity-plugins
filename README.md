...
Skills, scripts, and local credential files are shared. Codex MCP stays in `.mcp.json` with relative `./` paths. Cursor `mcp.json` starts `C:\Windows\System32\cmd.exe` plus `scripts/launch_cursor_mcp.cmd`, because plugin MCP spawn has an empty PATH. The wrapper restores Node and the Python launcher, then runs the Codex entrypoint from `${PLUGIN_ROOT}`. Regenerating Cursor manifests from the Codex index:
