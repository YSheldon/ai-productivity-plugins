...
class TestSubmissionScheduler:
    identity = "test-submission.retry"
    ...
    def __init__(
        ...
        *,
        ...
        self.cli_path = Path(cli_path).resolve(strict=False) if cli_path is not None else (Path(__file__).resolve().parent / "test_submission_cli.py").resolve()
        self.python_executable = Path(python_executable).resolve(strict=False) if python_executable is not None else Path(sys.executable).resolve(strict=False)
    ...
    def remove(self, *, mode: str = "auto") -> dict[str, Any]:
        ...
        if resolved == "windows":
            ...
            lines = [line for line in (current.stdout or "").splitlines() if self.cron_marker not in line]
    ...
    def _install_systemd(self) -> dict[str, Any]:
        ...
        service_path.write_text(
            "\n".join(("[Unit]", "Description=Test submission retry", "", "[Service]", "Type=oneshot", f"ExecStart={shlex.join(self.scheduled_command)}", "")),
            encoding="utf-8",
        )
        timer_path.write_text(
            "\n".join(("[Unit]", "Description=Test submission retry timer", "", "[Timer]", f"OnUnitActiveSec={self.poll_minutes}min", "Persistent=false", f"Unit={self.identity}.service", "", "[Install]", "WantedBy=timers.target", "")),
            encoding="utf-8",
        )
    ...
    def _install_cron(self) -> dict[str, Any]:
        current = self.runner(["crontab", "-l"], None, None)
        lines = [] if current.returncode != 0 else [line for line in (current.stdout or "").splitlines() if self.cron_marker not in line]
        lines.append(f"*/{self.poll_minutes} * * * * {' '.join(shlex.quote(part) for part in self.scheduled_command)} {self.cron_marker}")
        self.runner(["crontab", "-"], None, "\n".join(lines) + "\n")
        return {"installed": True}

    def _status_cron(self) -> dict[str, Any]:
        current = self.runner(["crontab", "-l"], None, None)
        installed = current.returncode == 0 and self.cron_marker in (current.stdout or "")
        return {
