...
def default_config_path() -> Path:
    explicit = str(os.environ.get("RELEASE_GATE_CONFIG") or "").strip()
    if explicit:
    ...
    return (root / "release-gate" / "config.json").resolve(strict=False)


def load_config(path: str | Path) -> ReleaseGateConfig:
    ...
    return ReleaseGateConfig(
        mail_account=MailAccountConfig(profile=_string(mail, "profile"), email=_email(_string(mail, "email"), "mail_account.email")),
        release_gate_group=_email(_string(payload, "release_gate_group"), "release_gate_group"),
        release_group=_email(_string(payload, "release_group"), "release_group"),
        ...
        product_gate=ProductGateConfig(config_path=_path(product_gate, "config_path"), command=_command(product_gate, "command")),
        policy_profile=_string(policy, "profile"),
        required_checks=_required_checks(policy),
        enabled_optional_checks=tuple(_string_list(policy, "enabled_optional_checks")),
    )


def missing_canonical_required_checks(required_checks: tuple[str, ...] | list[str]) -> tuple[str, ...]:
    normalized = tuple(str(item).strip() for item in required_checks if str(item).strip())
    return tuple(check for check in CANONICAL_REQUIRED_CHECKS if check not in normalized)


def _required_checks(payload: dict[str, Any]) -> tuple[str, ...]:
    checks = tuple(_string_list(payload, "required_checks"))
    missing = missing_canonical_required_checks(checks)
    if missing:
        raise ConfigError("policy.required_checks is missing canonical required checks: " + ", ".join(missing))
    return checks
