...
5. Build Manifest-R into a new, non-existent output path and run the release gate. The first build stages, durably copies, verifies SHA1/SHA256, and atomically publishes the complete directory. An exact same-path replay verifies and returns the existing frozen output; a conflicting path or content blocks. Any first-build failure cleans staging and leaves `RELEASE_PREPARING`. File drift, omissions, extras, signature failure, scan failure, or approval mismatch blocks release. Authenticode must match an exact configured certificate-thumbprint allowlist.
