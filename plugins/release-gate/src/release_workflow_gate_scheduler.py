...
def run_command(command: list[str], cwd: str | None = None, input_text: str | None = None) -> subprocess.CompletedProcess[str]:
    options: dict[str, Any] = {"cwd": cwd, "input": input_text, "capture_output": True, "check": False, "shell": False}
...
class ReleaseGateScheduler:
    ...
    def __init__(
        ...
        *,
        ...
        self.which = which
        self.runner = runner
        self.user_config_root = Path(user_config_root if user_config_root is not None else os.environ.get("XDG_CONFIG_HOME") or Path.home() / ".config").resolve(strict=False)
        self.cli_path = (Path(__file__).resolve().parent / "release_gate_cli.py").resolve()
        self.scheduled_command = [sys.executable, str(self.cli_path), "--config", str(self.config_path), "run-once"]
    ...
    def _install_windows(self) -> dict[str, Any]:
        ...
        command = ["schtasks", "/Create", "/TN", self.identity, "/TR", subprocess.list2cmdline(self.scheduled_command), "/SC", schedule_type, "/MO", modifier, "/F"]
    ...
    def _status_windows(self) -> dict[str, Any]:
        ...
        return {"status": "ready" if installed else "CAPABILITY_BLOCKED", "mode": "windows", "installed": installed, "overlap_policy_verified": overlap == "IgnoreNew", "misfire_policy_verified": start_when_available is False}

    def _remove_windows(self) -> dict[str, Any]:
        self._require_success(self.runner(["schtasks", "/Delete", "/TN", self.identity, "/F"], str(self.state_dir), None), "SCHEDULER_REMOVE_FAILED")
        return {"removed": True}
    ...
    def _install_systemd(self) -> dict[str, Any]:
        ...
        service_path.write_text(
            "\n".join(["[Unit]", f"Description=Release gate scheduler for {self.identity}", "", "[Service]", "Type=oneshot", f"ExecStart={shlex.join(self.scheduled_command)}", ""]) + "\n",
            encoding="utf-8",
        )
        timer_path.write_text(
            "\n".join(["[Unit]", f"Description=Release gate timer for {self.identity}", "", "[Timer]", f"OnUnitActiveSec={self.poll_minutes}min", "Persistent=false", "", "[Install]", "WantedBy=timers.target", ""]) + "\n",
            encoding="utf-8",
        )
        for command in (["systemctl", "--user", "daemon-reload"], ["systemctl", "--user", "enable", "--now", f"{self.identity}.timer"]):
            self._require_success(self.runner(command, str(self.state_dir), None), "SCHEDULER_INSTALL_FAILED")
    ...
    def _remove_systemd(self) -> dict[str, Any]:
        for command in (["systemctl", "--user", "disable", "--now", f"{self.identity}.timer"], ["systemctl", "--user", "daemon-reload"]):
    ...
    def _status_cron(self) -> dict[str, Any]:
        ...
        return {"status": "ready" if count == 1 else "CAPABILITY_BLOCKED", "mode": "cron", "installed": count == 1, "entry_count": count, "overlap_policy": "kernel_run_lock", "misfire_policy_verified": True}
    ...
    def _systemd_user_available(self) -> bool:
        return self.runner(["systemctl", "--user", "show-environment"], str(self.state_dir), None).returncode == 0
