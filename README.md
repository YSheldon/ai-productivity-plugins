...
Skills, scripts, and local credential files are shared. Codex MCP stays in `.mcp.json` with relative `./` paths. Cursor `mcp.json` starts `C:\Windows\System32\cmd.exe` plus `scripts/launch_cursor_mcp.cmd`. The wrapper restores Node and the Python launcher, then cds to the plugin root; do not set `cwd` to `${PLUGIN_ROOT}` because Cursor leaves that token unexpanded and Node reports it as spawn ENOENT. Regenerating Cursor manifests from the Codex index:
