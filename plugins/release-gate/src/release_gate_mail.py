...
class ReleaseGateMailError(RuntimeError):
    pass
...
def resolve_locked_entrypoint(
    ...
    *,
    ...
    if sha256_file(lock_path) != str(dependency_lock_sha256).strip().lower():
        raise ReleaseGateMailError("dependency lock drift was detected")
    payload = json.loads(lock_path.read_text(encoding="utf-8"))
    plugins = payload.get("plugins") if isinstance(payload, Mapping) else None
    if not isinstance(plugins, list):
        raise ReleaseGateMailError("dependency lock must contain plugins")
    ...
    for plugin in plugins:
        ...
        if Path(str(plugin.get("plugin_root") or "")).as_posix() != expected_root.as_posix():
            raise ReleaseGateMailError(f"dependency lock {plugin_name} root is invalid")
        for entrypoint in plugin.get("entrypoints", []):
            ...
            if sha256_file(resolved) != expected_digest:
                raise ReleaseGateMailError("locked runtime entrypoint drift was detected")
            return resolved
    raise ReleaseGateMailError(f"dependency lock does not include {plugin_name}")
...
def decode_machine_event(text: str) -> dict[str, Any]:
    try:
        ...
        raise ReleaseGateMailError("machine event block is missing") from exc
    padding = "=" * (-len(encoded) % 4)
    payload = json.loads(base64.urlsafe_b64decode((encoded + padding).encode("ascii")).decode("utf-8"))
    if not isinstance(payload, dict):
        raise ReleaseGateMailError("machine event must be one object")
    return payload
...
class ImapSmtpMailCliGateway:
    ...
    def _invoke(self, tool: str, arguments: Mapping[str, Any]) -> dict[str, Any]:
        ...
        if completed.returncode != 0:
            raise ReleaseGateMailError((completed.stderr or completed.stdout or "mail CLI failed").strip())
        envelope = json.loads(completed.stdout)
        if not isinstance(envelope, dict) or envelope.get("ok") is not True:
            raise ReleaseGateMailError(str(envelope))
        result = envelope.get("result")
        if not isinstance(result, dict):
            raise ReleaseGateMailError("mail CLI response is missing result")
        return dict(result)
    ...
    def list_accounts(self) -> list[dict[str, Any]]:
        ...
        return [dict(item) for item in accounts if isinstance(accounts, list) and isinstance(item, Mapping)]
...
class ProductGateCliGateway:
    ...
    def call(self, operation: str, payload: Mapping[str, Any]) -> dict[str, Any]:
        ...
        if completed.returncode not in {0, 3}:
            raise ReleaseGateMailError((completed.stderr or completed.stdout or "product-release-gate call failed").strip())
        result = json.loads(completed.stdout)
        if not isinstance(result, dict):
            raise ReleaseGateMailError("product-release-gate returned invalid JSON")
        return result


def locked_mail_gateway(dependency_lock: str | Path, *, dependency_lock_sha256: str, runner: Runner = subprocess.run) -> ImapSmtpMailCliGateway:
