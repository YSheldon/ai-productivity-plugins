...
def test_setup_writes_locked_config_and_runs_once() -> None:
    with tempfile.TemporaryDirectory() as temporary:
            ...
            json.dumps(
                {
                    "plugins": [
                        ...
                        },
                        {
                            ...
                            "entrypoints": [{"path": "plugins/product-release-gate/src/release_gate_cli.py", "sha256": hashlib.sha256(product_cli.read_bytes()).hexdigest()}],
                        },
                    ]
                }
            ),
        ...
        result = setup.run(non_interactive=True, scheduler_mode="auto", provided={"mail_profile": "release-gate"})
        assert result["status"] == "ready"
        config = json.loads(config_path.read_text(encoding="utf-8"))
        assert config["mail_account"]["profile"] == "release-gate"
