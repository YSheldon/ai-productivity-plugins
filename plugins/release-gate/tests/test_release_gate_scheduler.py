...
MODULE_PATH = SRC_ROOT / "release_workflow_gate_scheduler.py"


def _load_module():
    sys.path.insert(0, str(SRC_ROOT))
    spec = importlib.util.spec_from_file_location("release_workflow_gate_scheduler", MODULE_PATH)
    module = importlib.util.module_from_spec(spec)
    sys.modules.setdefault("release_workflow_gate_scheduler", module)
...
def test_windows_scheduler_install_and_status(tmp_path: Path) -> None:
    ...
    scheduler = module.ReleaseGateScheduler(
