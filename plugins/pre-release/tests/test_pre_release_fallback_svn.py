...
def test_plain_human_mail_is_discoverable_but_cannot_release_without_manifest(tmp_path: Path) -> None:
    ...
    tested_material_dir = tmp_path / "tested-plain"
    tested_material_dir.mkdir()
    with pytest.raises(PreReleaseError, match="full verified Manifest-S") as exc:
    ...
    assert exc.value.code == "MISSING_MANIFEST_S"
    assert controller.mail_gateway.sent == []
