...
- `remotex`: a credential-reference-first remote operations plugin for SSH, Windows RDP, vSphere/ESXi, and VMware Workstation. It adds native Windows and POSIX script transport, verified SFTP, resumable tasks, managed SSH host keys, hash-linked local audit records, and renewable FIFO leases shared across every access path to the same VM.
...
After installing RemoteX, copy `plugins/remotex/config/config.example.json` to `~/.config/remotex/config.json` and replace the example endpoints with local profile values. Keep only credential references in this file. Set `platform` and host-key policy for SSH, and give SSH, RDP, vSphere, and VMware profiles the same `queue_resource` when they address the same VM.

Run `remotex_status` with the selected profile before connecting. It separates selected-profile readiness from aggregate status, missing clients, credential references, host-key governance, and local queue state. Queue claims use bounded renewable leases; expiry releases ownership but never silently assigns a waiter. The old SSH config remains readable in compatibility mode when no RemoteX config exists. See `plugins/remotex/README.md` for execution limits, verified transfer, resumable tasks, audit behavior, and safety boundaries.
